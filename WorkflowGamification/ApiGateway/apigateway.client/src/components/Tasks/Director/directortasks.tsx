...
import { sendGetRequestWithId, sendPatchRequest } from '../../sendrequest';
import { DoneTasksDirector } from './donetasksdirector';
import { DoingTasksDirector } from './doingtasksdirector';
import { UntakenTasksDirector } from './untakentasksdirector';
    ...
    const handleNewTask = (e: React.FormEvent<HTMLFormElement>) => {
      ...
      console.log(creatingTask);
          try {
              ...
              console.log('Ответ сервера:', sendPatchRequest("https:localhost:7256/api/Job/AddAnswerToJob", creatingTask, accessToken));
              fetchTasks();
          } catch (error) {
       ...
       const fetchTasks = async () => {
        try {
          const accessToken = localStorage.getItem('accessToken');
          const response = await sendGetRequestWithId("https://localhost:7256/api/Job/GetUnfinishedJobsInDepartment", accessToken, "1");
