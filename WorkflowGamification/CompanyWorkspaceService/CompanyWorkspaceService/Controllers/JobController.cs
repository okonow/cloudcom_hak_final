...
namespace CompanyWorkspaceService.Controllers
{
    ...
    public class JobController(ISender sender) : BaseController(sender)
    {
        ...
        [HttpGet]
        public async Task<IList<JobMainInfoVM>> GetUnfinishedJobsInDepartmentAsync([FromHeader] Guid departmentId)
        ...
        [HttpPatch]
        public async Task AddEmployeeTJob([FromBody] AddEmployeeToJobCommand command)
            => await _sender.Send(command);
